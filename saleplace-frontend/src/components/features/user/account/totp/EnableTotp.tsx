...
import { enableTotpSchema, type TypeEnableTotpSchema } from "@/schemas/user/enable-totp.chema"
...
export function EnableTotp() {
    ...
    const [enable, { loading: isLoadingEnable }] = useEnableTotpMutation({
        onCompleted(data) {
            if (!data?.enableTotp) return;

            client.cache.modify({
                id: client.cache.identify({
                    __typename: 'UserModel',
                    id: data.enableTotp.id
                }),
                fields: {
                    isTotpEnabled() {
                        return data.enableTotp.isTotpEnabled;
                    }
                }
            })
            setIsOpen(false)
